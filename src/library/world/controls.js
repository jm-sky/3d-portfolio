...
// import {PointerLockControls} from 'three-112/examples/jsm/controls/PointerLockControls.js';
...
export const controls = (function() {
  return {
    ...
    FPSControls: class {
      ...
      //-------------------------------
      _Init(params) {
        this._targetSelector = params._targetSelector || '#canvas';
        this._radius = 2;
        this._enabled = false;
        this._move = {
          forward: false,
        ...
        this._params._guiParams.camera = {
          acceleration_x: 250,
        };
  
        const rollup = this._params._gui.addFolder('Camera.FPS');
        rollup.add(this._params._guiParams.camera, "acceleration_x", 50.0, 25000.0).onChange(() => {
          const acc = this._params._guiParams.camera.acceleration_x;
          this._acceleration.set(acc, acc,acc);
        });
      ...
      _SetupPointerLock() {
        ...
        if (hasPointerLock) {
          ...
          document.querySelector(this._targetSelector).addEventListener('click', (event) => {
      ...
      update(timeInMs) {
        ...
        // console.log('[controls] params:', this._params);
        // console.log('[controls] controlObject:', controlObject);
        // // let [x, y, z] = controlObject.position;
        // let manager = this._params.entities.TerrainChunkManager[0];
        // let planes = Object.values(manager._chunks).map(c => c._plane);
